...
/*
 * This was taken from bc-1.0.6.  The author's copyright is below.
 *
 * $Id: number.c,v 0.3 2010/04/23 16:53:54 dfmay Exp dfmay $
 *
 * $Log: number.c,v $
 * Revision 0.3  2010/04/23 16:53:54  dfmay
 * I altered the functions to be ANSI C compliant (finally).
 *
 *
 */
...
/*
 * Much of this was taken from number.c from bc-1.0.6, but some of it was not.
 */
#include <stdio.h>
#include <string.h>
#include <number.h>
...
bc_num bc_new_num (int length, int scale)
{
  ...
  temp->n_scale = scale;
  temp->n_refs = 1;
  temp->n_ptr = (char *) malloc ((size_t)length + scale);
  if (temp->n_ptr == NULL)
    bc_out_of_memory ();
  temp->n_value = temp->n_ptr;
  memset (temp->n_ptr, 0, (size_t)length + scale);
  return temp;
}
  ...
  if (n1->n_sign != n2->n_sign) {
    ...
    switch (cmp_res) {
      ...
      case 0:
        ...
        memset (diff->n_value, 0, (size_t)res_scale + 1);
        break;
      case 1:
        /*
  ...
  if (n1->n_sign == n2->n_sign) {
    ...
    switch (cmp_res) {
      ...
      case 0:
        ...
        memset (sum->n_value, 0, (size_t)res_scale + 1);
        break;
      case 1:
        /*
         * n2 is less than n1, subtract n2 from n1. 
             ...
             int full_scale)
{
  ...
  if ((ulen + vlen) < mul_base_digits
      || ulen < MUL_SMALL_DIGITS || vlen < MUL_SMALL_DIGITS) {
  ...
  m1zero = (char)(bc_is_zero (u1) || bc_is_zero (v1));
  ...
  *prod = bc_new_num (prodlen, 0);
  ...
  _bc_shift_addsub (*prod, m2, n, (int)(d1->n_sign != d2->n_sign));
...
static void _one_mult (unsigned char *num, int size, int digit, unsigned char *result)
{
  ...
  if (digit == 0)
    ...
    if (digit == 1)
      memmove (result, num, (size_t)size);
    else {
      /*
       * Initialize 
       */
...
int bc_divide (bc_num n1, bc_num n2, bc_num *quot, int scale)
{
  ...
  if (n2->n_scale == 0) {
    if (n2->n_len == 1 && *n2->n_value == 1) {
      ...
      memset (&qval->n_value[n1->n_len], 0, (size_t)scale);
      memmove (qval->n_value, n1->n_value,
               (size_t)(n1->n_len + MIN (n1->n_scale, scale)));
      bc_free_num (quot);
      *quot = qval;
    }
  }
  ...
  if (scale1 < scale)
    ...
    extra = 0;
  num1 = (unsigned char *) malloc ((size_t)(n1->n_len + n1->n_scale + extra + 2));
  if (num1 == NULL)
    bc_out_of_memory ();
  memset (num1, 0, (size_t)(n1->n_len + n1->n_scale + extra + 2));
  memmove (num1 + 1, n1->n_value, (size_t)(n1->n_len + n1->n_scale));

  len2 = (unsigned int)(n2->n_len + scale2);
  num2 = (unsigned char *) malloc ((size_t)len2 + 1);
  if (num2 == NULL)
    bc_out_of_memory ();
  memmove (num2, n2->n_value, (size_t)len2);
  *(num2 + len2) = 0;
  n2ptr = num2;
  while (*n2ptr == 0) {
    n2ptr++;
   ...
   */
  if (len2 > len1 + scale) {
    qdigits = (unsigned int)scale + 1;
    zero = TRUE;
  }
  else {
    zero = FALSE;
    if (len2 > len1)
      qdigits = (unsigned int)scale + 1;             /* One for the zero integer part. */
    else
   ...
   */
  qval = bc_new_num ((int)(qdigits - scale), scale);
  memset (qval->n_value, 0, (size_t)qdigits);

  /*
   * Allocate storage for the temporary storage mval. 
   */
  mval = (unsigned char *) malloc ((size_t)len2 + 1);
  if (mval == NULL)
    bc_out_of_memory ();
  ...
  if (!zero) {
    ...
    if (norm != 1) {
      _one_mult (num1, (int)(len1 + scale1 + extra + 1), (int)norm, num1);
    ...
    while (qdig <= len1 + scale - len2) {
      ...
      if (*n2ptr == num1[qdig])
        ...
        qguess = (unsigned int)((num1[qdig] * 10 + num1[qdig + 1]) / *n2ptr);

      /*
       * Test qguess. 
       */
      if ((int)(n2ptr[1] * qguess) >
          (int)((num1[qdig] * 10 + num1[qdig + 1] - *n2ptr * qguess) * 10
          + num1[qdig + 2])) {
        qguess--;
        /*
         * And again. 
...
int bc_divmod (bc_num num1, bc_num num2, bc_num *quot, bc_num *rem, int scale)
{
   ...
   * Calculate it. 
   */
  (int)bc_divide (num1, num2, &temp, scale);
  if (quot)
...
int bc_raisemod (bc_num base, bc_num expo, bc_num mod, bc_num *result, int scale)
{
  ...
  if (exponent->n_scale != 0) {
    ...
    (int)bc_divide (exponent, _one_, &exponent, 0);  /*truncate */
  }
...
void bc_raise (bc_num num1, bc_num num2, bc_num *result, int scale)
{
   ...
   */
  if (neg) {
    (int)bc_divide (_one_, temp, result, rscale);
    bc_free_num (&temp);
  }
  else {
    bc_free_num (result);
...
int bc_sqrt (bc_num *num, int scale)
{
  ...
  while (!done) {
    ...
    (int)bc_divide (*num, guess, &guess, cscale);
   ...
   */
  bc_free_num (num);
  (int)bc_divide (guess, _one_, num, rscale);
  bc_free_num (&guess);
  bc_free_num (&guess1);
  bc_free_num (&point5);
  bc_free_num (&diff);
...
)
{
  ...
  if (bc_is_zero (num))
    ...
    (int)bc_divide (num, _one_, &int_part, 0);
    ...
    while (!bc_is_zero (int_part)) {
      (int)bc_modulo (int_part, base, &cur_dig, 0);
      ...
      temp->next = digits;
      digits = temp;
      (int)bc_divide (int_part, base, &int_part, 0);
    }
    ...
    if (num->n_scale > 0) {
      ...
      while (t_num->n_len <= num->n_scale) {
        ...
        fdigit = (int)bc_num2long (frac_part);
...
char *num2str (bc_num num)
{
  ...
  if (num->n_scale > 0)
    str = (char *) malloc ((size_t)(num->n_len + num->n_scale + 2 + signch));
  else
    str = (char *) malloc ((size_t)(num->n_len + 1 + signch));
  if (str == NULL)
    bc_out_of_memory ();
...
#ifdef HAVE_SNPRINTF
  (int)snprintf (bcnum_outstr, BCNUM_OUTSTRING_SIZE, "***Error: %s: ",
...
#ifdef HAVE_SNPRINTF
  (int)vsnprintf (bcnum_outstr + len, BCNUM_OUTSTRING_SIZE - 10, mesg, args);
#else
...
void rt_warn (char *mesg, ...)
{
...
#ifdef HAVE_SNPRINTF
  (int)snprintf (bcnum_outstr, BCNUM_OUTSTRING_SIZE, "***Warning: %s: ",
            bcnumErrMsg[bcnumError]);
#else
  ...
  len = strlen (bcnum_outstr);
#ifdef HAVE_SNPRINTF
  (int)vsnprintf (bcnum_outstr + len, BCNUM_OUTSTRING_SIZE - 12, mesg, args);
#else
...
char *bcnum_divide (char *n1, char *n2, int scale)
{
  ...
  status = bc_divide (num1, num2, &div, scale);
  pn_str (div);
  ...
  if (status == -1 || bcnumError != BCNUM_NOERROR)
    return 0;
  return bcnum_outstr;
}
...
char *bcnum_modulo (char *n1, char *n2, int scale)
{
  ...
  num1 = bc_copy_num (_zero_);
  num2 = bc_copy_num (_zero_);
  mod = bc_copy_num (_zero_);

  bc_str2num (&num1, n1, scale);
  bc_str2num (&num2, n2, scale);

  status = bc_modulo (num1, num2, &mod, scale);
  pn_str (mod);
...
char *bcnum_sqrt (char *n1, int scale)
{
  ...
  bcnum_outstr_ptr = 0;
  sqrt = bc_copy_num (_zero_);

  bc_str2num (&sqrt, n1, scale);

  status = bc_sqrt (&sqrt, scale);
  pn_str (sqrt);
...
int bcnum_isnearzero (char *n1, int scale)
{
  ...
  num1 = bc_copy_num (_zero_);
  bc_str2num (&num1, n1, scale);
  status = (int)bc_is_near_zero (num1, scale);
  bc_free_num (&num1);
  return status;
}
